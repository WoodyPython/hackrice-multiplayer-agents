...
- **Host controls disappeared:** account membership, confirmed by the server, replaces the obsolete requirement for a browser-stored host key. Workspace permissions reload when the account changes; rejected settings writes remove host controls.
...
- **Concurrent host edits could deadlock:** roster changes serialize on the workspace before locking individual members, preserving at least one host.
...
The browser automation tool reported no available browser, so no visual browser walkthrough was completed. The local `.env` also lacks `SUPABASE_URL`, `SUPABASE_PUBLISHABLE_KEY`, `SUPABASE_SECRET_KEY`, `VITE_SUPABASE_URL`, and `VITE_SUPABASE_PUBLISHABLE_KEY`; real provider login remains unverified. Supply the matching keys using `.env.example`, and rebuild after changing Vite variables. Never put a Supabase secret or service-role key in a `VITE_` variable.
