...
/**
 * What the seeded document holds: the fixture above with its CRLF line endings
 * normalised, per editor-text. Named rather than repeated so changing the
 * fixture cannot leave an assertion pointing at text that no longer exists.
 */
const NORMALISED_SOURCE = 'def first():\n    return 1\n\ndef second():\n    return 2\n';
...
describe('D07 owner apply', { timeout: 60_000 }, () => {
  ...
  it('marks accepted typing stale before persistence, including edit-and-undo', async () => {
    ...
    expect(peer.text.toString()).toBe(NORMALISED_SOURCE);
  });
  ...
  it('rejects typing queued after the Apply gate without modifying the candidate', async () => {
    ...
    try {
      ...
      expect(acquired.room.doc.getText(LIVE_TEXT_NAME).toString()).toBe(NORMALISED_SOURCE);
